...
static int verify_callback_nocheck(int ok, X509_STORE_CTX *store)
{
	return 1;
}
...
long check_host(SSL *ssl, const char *host)
{
...
static SSL_CTX *setup_client_ctx(char *cert_file)
{
	...
	if(cert_file!=NULL && *cert_file=='\0'){
		...
		SSL_CTX_set_verify(ctx,SSL_VERIFY_NONE,verify_callback_nocheck);
	}
...
size_t TLS_Send_Data(SSL *ssl,unsigned char *buf,size_t len)
{
...
/**
  ...
  @return <0 エラー
  */
size_t TLS_Recv_Data(SSL *ssl,unsigned char *buf,size_t len,int timeout,int *reason)
{
	int err;
	int	fd;
	struct timeval tv;
...
SSL *TLS_Connect(char *host,int port,int *reason)
{
	...
	err=check_host(ssl,host);
	if(err!=X509_V_OK){
		SSL_shutdown(ssl);
		SSL_free(ssl);
		//ERR_remove_state(0); deplicated
		if(reason!=NULL){
			*reason=-700;
		}
		return NULL;
...
err:
	...
	delete_SSL(key);
err2:
	//ERR_remove_state(0);deplicated //against Memory Leak 
#ifdef DEBUG
