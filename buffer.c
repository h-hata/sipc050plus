...
static void params_to_asc(URIPARAM *param,char *buff,int type);
...
int MakeURItoASC(URI *uri,char *buff,int type)
{
	...
	*buff='\0';
	...
	strcat(buff,line);
	*line='\0';
	params_to_asc(&uri->param,line,INSIDE);
	strcat(buff,line);
	if(brace!=0) strcat(buff,">");//Mar.11.2005パラメータは括弧外に出す
	params_to_asc(&uri->param,line,OUTSIDE);
	strcat(buff,line);
	strcat(buff,uri->aux);
...
int MakeAnonymous(URI *uri,char *buff)
{
	...
	*buff='\0';
	...
	strcat(buff,line);
	*line='\0';
	params_to_asc(&uri->param,line,ALL);
	strcat(buff,line);
...
{	
	...
	*buff='\0';
	...
	for(;via!=NULL;via=via->next){
		...
		strcat(buff,line);
		*line='\0';
		params_to_asc(&via->param,line,ALL);
		strcat(buff,line);
		strcat(buff,"\r\n");
	}
	//----------------------------------------------------MaxForwards
	...
	*buff='\0';
	...
	if(type==ALL||type==INSIDE){
		if(*param->maddr){
			...
			if(strlen(buff)+strlen(line)<sizeof(buff))
				strcat(buff,line);
		}
